...
function Posts({ userData = null }) {
  ...
  useEffect(() => {
    let parr = [];
    const unsub = database.posts.orderBy('createdAt', 'desc').onSnapshot(querySnapshot => {  // onSnapshot looks at realtime changes
      parr = [];
        ...
        <div className='video-container' id='video-container'>
          {
            posts.map((post) => (
              <React.Fragment key={post.postId}>
                <div className='videos' style={{ display: 'flex' }}>
